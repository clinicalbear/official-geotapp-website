// Overview: pricing-promo-codes.test.js
// Module: test
...
test('pricing page removes Flow, FortyX, and Zenith promo codes', () => {
  const source = fs.readFileSync(pricingPagePath, 'utf8');

  assert.doesNotMatch(source, /promoCode:\s*'FLOW25'/);
  assert.doesNotMatch(source, /promoCode:\s*'FORTYX25'/);
  assert.doesNotMatch(source, /promoCode:\s*'ZENITH30'/);
});
