...
// condizionale del testo pan-UE. In Germania la cogestione non e' eventuale:
// § 87 Abs. 1 Nr. 6 BetrVG guarda alla sola idoneita' oggettiva al controllo,
// quindi il sistema e' mitbestimmungspflichtig e, dove esiste un Betriebsrat,
// senza Betriebsvereinbarung non puo' essere attivato (revisore legale DE).
const CODETERM_OVERRIDE: Record<string, Partial<Record<InfLocale, string>>> = {
  DE: {
    it: "L'introduzione e l'utilizzo di questo sistema sono soggetti alla cogestione del consiglio aziendale (Betriebsrat) ai sensi del § 87 Abs. 1 Nr. 6 BetrVG. Rileva la sola idoneità oggettiva del sistema al controllo del comportamento e della prestazione, non la finalità perseguita: vi rientra anche la rilevazione puntuale alla timbratura di entrata e uscita. Dove esiste un Betriebsrat, prima dell'attivazione del sistema va stipulata una Betriebsvereinbarung: senza di essa il sistema non può essere impiegato. In assenza di Betriebsrat viene meno la cogestione, non gli altri obblighi in materia di protezione dei dati.",
    en: 'The introduction and use of this system are subject to the co-determination of the works council (Betriebsrat) under § 87 Abs. 1 Nr. 6 BetrVG. What matters is solely the system’s objective capability of monitoring behaviour and performance, not the purpose pursued: point-in-time recording at clock-in and clock-out is covered as well. Where a works council exists, a works agreement (Betriebsvereinbarung) must be concluded before the system is activated; without it the system may not be used. Where no works council exists, co-determination does not apply, but the remaining data protection obligations do.',
    de: 'Die Einführung und Anwendung dieses Systems unterliegen der Mitbestimmung des Betriebsrats nach § 87 Abs. 1 Nr. 6 BetrVG. Maßgeblich ist allein die objektive Eignung des Systems zur Überwachung von Verhalten und Leistung, nicht der verfolgte Zweck: Erfasst ist damit auch die punktuelle Erhebung beim Ein- und Ausstempeln. Besteht ein Betriebsrat, ist vor der Aktivierung des Systems eine Betriebsvereinbarung abzuschließen; ohne diese darf das System nicht eingesetzt werden. Besteht kein Betriebsrat, entfällt die Mitbestimmung, nicht jedoch die übrigen datenschutzrechtlichen Pflichten.',
    fr: 'L’introduction et l’utilisation de ce système sont soumises à la cogestion du comité d’entreprise (Betriebsrat) au titre du § 87 Abs. 1 Nr. 6 BetrVG. Seule compte l’aptitude objective du système à contrôler le comportement et la performance, non la finalité poursuivie : le relevé ponctuel au pointage d’entrée et de sortie en relève également. Lorsqu’un comité d’entreprise existe, un accord d’entreprise (Betriebsvereinbarung) doit être conclu avant l’activation du système ; sans cet accord, le système ne peut pas être utilisé. En l’absence de comité d’entreprise, la cogestion ne s’applique pas, mais les autres obligations en matière de protection des données demeurent.',
    es: 'La introducción y el uso de este sistema están sujetos a la cogestión del comité de empresa (Betriebsrat) conforme al § 87 Abs. 1 Nr. 6 BetrVG. Lo determinante es únicamente la idoneidad objetiva del sistema para controlar el comportamiento y el rendimiento, no la finalidad perseguida: también queda incluido el registro puntual al fichar la entrada y la salida. Cuando existe un comité de empresa, antes de activar el sistema debe celebrarse un convenio de empresa (Betriebsvereinbarung); sin él, el sistema no puede emplearse. Si no existe comité de empresa, decae la cogestión, pero no las demás obligaciones de protección de datos.',
    nl: 'De invoering en het gebruik van dit systeem zijn onderworpen aan de medezeggenschap van de ondernemingsraad (Betriebsrat) op grond van § 87 Abs. 1 Nr. 6 BetrVG. Bepalend is uitsluitend de objectieve geschiktheid van het systeem om gedrag en prestaties te controleren, niet het nagestreefde doel: ook de puntsgewijze registratie bij het in- en uitklokken valt eronder. Bestaat er een ondernemingsraad, dan moet vóór de activering van het systeem een ondernemingsovereenkomst (Betriebsvereinbarung) worden gesloten; zonder die overeenkomst mag het systeem niet worden ingezet. Bestaat er geen ondernemingsraad, dan vervalt de medezeggenschap, maar niet de overige verplichtingen inzake gegevensbescherming.',
    pt: 'A introdução e a utilização deste sistema estão sujeitas à cogestão da comissão de trabalhadores (Betriebsrat) nos termos do § 87 Abs. 1 Nr. 6 BetrVG. Releva apenas a idoneidade objetiva do sistema para controlar o comportamento e o desempenho, não a finalidade prosseguida: abrange também o registo pontual na marcação de entrada e de saída. Existindo uma comissão de trabalhadores, deve ser celebrado um acordo de empresa (Betriebsvereinbarung) antes da ativação do sistema; sem ele, o sistema não pode ser utilizado. Não existindo comissão de trabalhadores, cessa a cogestão, mas não as restantes obrigações em matéria de proteção de dados.',
    da: 'Indførelsen og brugen af dette system er underlagt Betriebsrats medbestemmelse i henhold til § 87 Abs. 1 Nr. 6 BetrVG. Afgørende er alene systemets objektive egnethed til at kontrollere adfærd og præstation, ikke det forfulgte formål: også den punktvise registrering ved ind- og udstempling er omfattet. Findes der et Betriebsrat, skal der indgås en virksomhedsaftale (Betriebsvereinbarung), før systemet aktiveres; uden den må systemet ikke anvendes. Findes der intet Betriebsrat, bortfalder medbestemmelsen, men ikke de øvrige databeskyttelsesretlige forpligtelser.',
    sv: 'Införandet och användningen av detta system omfattas av företagsrådets (Betriebsrat) medbestämmande enligt § 87 Abs. 1 Nr. 6 BetrVG. Avgörande är enbart systemets objektiva lämplighet att kontrollera beteende och prestation, inte det eftersträvade syftet: även den punktvisa registreringen vid in- och utstämpling omfattas. Finns ett företagsråd ska ett företagsavtal (Betriebsvereinbarung) ingås innan systemet aktiveras; utan ett sådant får systemet inte användas. Finns inget företagsråd bortfaller medbestämmandet, men inte övriga skyldigheter enligt dataskyddslagstiftningen.',
    nb: 'Innføringen og bruken av dette systemet er underlagt medbestemmelsen til Betriebsrat etter § 87 Abs. 1 Nr. 6 BetrVG. Avgjørende er utelukkende systemets objektive egnethet til å kontrollere atferd og ytelse, ikke det formålet som forfølges: også den punktvise registreringen ved inn- og utstempling omfattes. Finnes det et Betriebsrat, skal det inngås en virksomhetsavtale (Betriebsvereinbarung) før systemet aktiveres; uten en slik avtale kan systemet ikke tas i bruk. Finnes det ikke noe Betriebsrat, faller medbestemmelsen bort, men ikke de øvrige pliktene etter personvernlovgivningen.',
    ru: 'Внедрение и использование этой системы подлежат соучастию производственного совета (Betriebsrat) согласно § 87 Abs. 1 Nr. 6 BetrVG. Решающей является исключительно объективная пригодность системы для контроля поведения и результативности, а не преследуемая цель: сюда относится и разовая фиксация при отметке прихода и ухода. При наличии производственного совета до активации системы должно быть заключено производственное соглашение (Betriebsvereinbarung); без него система не может применяться. При отсутствии производственного совета соучастие отпадает, но остальные обязанности по защите данных сохраняются.',
  },
};
...
function codetermSezione(locale: InfLocale, i: InfInputs, codiceISO?: string): { titolo: string; testo: string } | null {
  ...
  const iso = codiceISO!.toUpperCase();
  const ov = CODETERM_OVERRIDE[iso];
  const testo = ov?.[locale] ?? ov?.en ?? sec.testo;
  return { titolo: sec.titolo, testo: testo.replace(/\{ref\}/g, ref).replace(/\{paese\}/g, i.paese) };
}
