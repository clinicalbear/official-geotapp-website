...
const FAQ: Record<string, FaqItem[]> = {
  ...
  en: [
    { q: 'What is the main difference between GeoTapp and Zucchetti?', a: 'Zucchetti is a workforce management suite, from attendance to payroll, where clock-in is one module among many. GeoTapp seals field work, turning every job into a sealed document the client can verify. One closes the month in the back office, the other defends an invoice.' },
  ...
  de: [
    { q: 'Was ist der Hauptunterschied zwischen GeoTapp und Zucchetti?', a: 'Zucchetti ist eine Suite für die Personalverwaltung, von der Zeiterfassung bis zur Lohnabrechnung, und die Stempelung ist eines von vielen Modulen. GeoTapp versiegelt die Arbeit vor Ort und macht aus jedem Einsatz ein versiegeltes Dokument, das der Auftraggeber selbst prüfen kann. Das eine schließt den Monat in der Verwaltung ab, das andere verteidigt eine Rechnung.' },
  ...
  es: [
    { q: '¿Cuál es la principal diferencia entre GeoTapp y Zucchetti?', a: 'Zucchetti es una suite de gestión de personal, del fichaje a la nómina, donde el registro horario es un módulo más. GeoTapp sella el trabajo en campo y convierte cada intervención en un documento sellado que el cliente verifica por su cuenta. El primero cierra el mes en la oficina, el segundo defiende una factura.' },
  ...
  nl: [
    { q: 'Wat is het belangrijkste verschil tussen GeoTapp en Zucchetti?', a: 'Zucchetti is een suite voor personeelsbeheer, van tijdregistratie tot salarisverwerking, waarin het inklokken een van de vele modules is. GeoTapp verzegelt het werk op locatie en maakt van elke opdracht een verzegeld document dat de opdrachtgever zelf kan controleren. De eerste sluit de maand af op kantoor, de tweede verdedigt een factuur.' },
...
const T: Record<string, Copy> = {
  it: {
    ...
    desc: 'Zucchetti gestisce il personale di mezza Italia, con badge, geofence e cedolini dentro un ecosistema solo. GeoTapp fa una cosa sola, sigilla l\'intervento sul campo con GPS verificato, foto sigillate e un report che il committente controlla da solo.',
    summary: 'In sintesi:',
  ...
  es: {
    ...
    desc: 'Zucchetti gestiona personal, tarjetas, geofence y nóminas dentro de un único ecosistema. GeoTapp hace una sola cosa, sella la intervención en campo con GPS verificado, fotos selladas y un informe que el cliente comprueba por su cuenta.',
    summary: 'En resumen:',
  ...
  pt: {
    ...
    desc: 'A Zucchetti gere pessoal, cartões, geofence e recibos dentro de um único ecossistema. A GeoTapp faz uma coisa só, sela a intervenção no terreno com GPS verificado, fotos seladas e um relatório que o cliente confere sozinho.',
    summary: 'Em resumo:',
