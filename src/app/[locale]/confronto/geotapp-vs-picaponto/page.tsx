...
const META: Record<string, { title: string; description: string }> = {
  ...
  en: { title: 'GeoTapp vs PicaPonto - Comparison 2026 | GeoTapp', description: 'GeoTapp vs PicaPonto: recording or proving? Compare pricing, clock-in methods, compliance and tamper-evident reports the client can verify independently.' },
  de: { title: 'GeoTapp vs PicaPonto - Vergleich 2026 | GeoTapp', description: 'GeoTapp vs PicaPonto: erfassen oder beweisen? Vergleich von Preis, Stempelmethoden, Compliance und versiegelten, vom Auftraggeber verifizierbaren Berichten.' },
  fr: { title: 'GeoTapp vs PicaPonto - Comparaison 2026 | GeoTapp', description: 'GeoTapp vs PicaPonto : enregistrer ou prouver ? Comparez prix, methodes de pointage, conformite et rapports scellés verifiables par le client.' },
  es: { title: 'GeoTapp vs PicaPonto - Comparacion 2026 | GeoTapp', description: 'GeoTapp vs PicaPonto: registrar o demostrar? Compara precio, metodos de fichaje, cumplimiento e informes sellados verificables por el cliente.' },
...
const FAQ: Record<string, FaqItem[]> = {
  ...
  nl: [
    ...
    { q: 'Kan de klant de rapporten verifieren?', a: 'PicaPonto genereert interne rapporten voor administratie en loon. GeoTapp genereert een rapport met een cryptografisch zegel dat de klant onafhankelijk verifieert, zonder account en zonder het bedrijf te vertrouwen. Een registratie die de werkgever kan corrigeren heeft niet hetzelfde gewicht als een verzegeld rapport.' },
  ],
  da: [
...
const T: Record<string, Copy> = {
  ...
  en: {
    ...
    summary: 'Bottom line:',
    summaryText: 'PicaPonto is a great attendance system for companies that need to record hours cheaply, with many clock-in methods and a strong compliance angle. GeoTapp is for companies that need to show their client verifiable proof of the work, with a tamper-evident report the client checks independently.',
    noteTitle: 'The question that changes the choice',
    noteText: 'PicaPonto answers "is my record in order if the inspector shows up?". GeoTapp answers "how do I show my client verifiable proof of the work?". They sound like the same question and they are not: an attendance record is for the State and payroll, a tamper-evident report is for that Friday-evening phone call where the client disputes the invoice. Neither is better, they solve different losses.',
  ...
  de: {
    ...
    summary: 'Fazit:',
    summaryText: 'PicaPonto ist ein sehr gutes Anwesenheitssystem fur Unternehmen, die Stunden gunstig erfassen mussen, mit vielen Stempelmethoden und starkem Compliance-Fokus. GeoTapp ist fur Unternehmen, die ihrem Auftraggeber uberprufbare Nachweise der Arbeit vorlegen mussen, mit einem versiegelten Bericht, den der Auftraggeber selbst pruft.',
    noteTitle: 'Die Frage, die die Wahl entscheidet',
    noteText: 'PicaPonto beantwortet "ist meine Aufzeichnung in Ordnung, wenn die Prufung kommt?". GeoTapp beantwortet "wie lege ich meinem Auftraggeber überprüfbare Nachweise der Arbeit vor?". Das klingt nach derselben Frage und ist es nicht: eine Zeiterfassung ist fur den Staat und die Lohnabrechnung, ein versiegelter Bericht ist fur den Freitagabend-Anruf, in dem der Kunde die Rechnung anficht. Keiner ist besser, sie losen unterschiedliche Verluste.',
  ...
  fr: {
    ...
    summary: 'En resume :',
    summaryText: 'PicaPonto est un excellent systeme de pointage pour ceux qui doivent enregistrer les heures a bas cout, avec de nombreuses methodes et un fort levier de conformite. GeoTapp est pour ceux qui doivent montrer a leur donneur d ordre des preuves verifiables du travail, avec un rapport scellé que le client controle lui-meme.',
    noteTitle: 'La question qui change le choix',
    noteText: 'PicaPonto repond a "mon registre est-il en ordre si l inspection arrive ?". GeoTapp repond a "comment montrer au client des preuves verifiables du travail ?". Cela semble la meme question et ce n en est pas une : un registre de presence est pour l Etat et la paie, un rapport scellé est pour cet appel du vendredi soir ou le client conteste la facture. Aucun n est meilleur, ils resolvent des pertes differentes.',
  ...
  es: {
    ...
    summary: 'En resumen:',
    summaryText: 'PicaPonto es un excelente sistema de fichaje para quien necesita registrar las horas a bajo coste, con muchos metodos y una fuerte palanca de cumplimiento. GeoTapp es para quien necesita mostrar a su cliente pruebas verificables del trabajo, con un informe sellado que el cliente controla por si mismo.',
    noteTitle: 'La pregunta que cambia la eleccion',
    noteText: 'PicaPonto responde a "tengo el registro en orden si llega la inspeccion?". GeoTapp responde a "como muestro al cliente pruebas verificables del trabajo?". Parecen la misma pregunta y no lo son: un registro de presencia es para el Estado y la nomina, un informe sellado es para esa llamada del viernes por la tarde en la que el cliente discute la factura. Ninguno es mejor, resuelven perdidas distintas.',
  ...
  pt: {
    ...
    summary: 'Em resumo:',
    summaryText: 'O PicaPonto e um otimo sistema de assiduidade para quem precisa de registar as horas a baixo custo, com muitos metodos e uma forte alavanca de conformidade. A GeoTapp e para quem precisa de mostrar ao cliente provas verificáveis do trabalho, com um relatorio selado que o cliente controla sozinho.',
    noteTitle: 'A pergunta que muda a escolha',
    noteText: 'O PicaPonto responde a "tenho o registo em ordem se chegar a inspecao?". A GeoTapp responde a "como mostro ao cliente provas verificaveis do trabalho?". Parecem a mesma pergunta e nao sao: um registo de presenca e para o Estado e o salario, um relatorio selado e para aquele telefonema de sexta a noite em que o cliente contesta a fatura. Nenhum e melhor, resolvem perdas diferentes.',
  ...
  nl: {
    ...
    summary: 'Kort gezegd:',
    summaryText: 'PicaPonto is een uitstekend aanwezigheidssysteem voor wie de uren goedkoop moet registreren, met veel klokmethodes en een sterke nalevingshoek. GeoTapp is voor wie zijn opdrachtgever verifieerbaar bewijs van het werk moet laten zien, met een verzegeld rapport dat de klant zelf controleert.',
    noteTitle: 'De vraag die de keuze bepaalt',
    noteText: 'PicaPonto beantwoordt "is mijn registratie in orde als de inspectie komt?". GeoTapp beantwoordt "hoe laat ik de klant verifieerbaar bewijs van het werk zien?". Ze klinken als dezelfde vraag en dat zijn ze niet: een aanwezigheidsregistratie is voor de Staat en het loon, een verzegeld rapport is voor dat telefoontje op vrijdagavond waarin de klant de factuur betwist. Geen van beide is beter, ze lossen verschillende verliezen op.',
