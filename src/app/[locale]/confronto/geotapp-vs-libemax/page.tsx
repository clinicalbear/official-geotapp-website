...
const META: Record<string, { title: string; description: string }> = {
  ...
  fr: { title: 'GeoTapp vs Libemax - Comparaison 2026 | GeoTapp', description: 'GeoTapp vs Libemax : geofence ou anti-spoofing ? Comparez GPS vérifié, rapports scellés cryptographiquement et preuves photographiques dont toute modification est détectable.' },
  es: { title: 'GeoTapp vs Libemax - Comparación 2026 | GeoTapp', description: 'GeoTapp vs Libemax: ¿geofence o anti-spoofing? Compara GPS verificado, informes sellados criptográficamente y pruebas fotográficas cuya modificación es detectable.' },
...
const FAQ: Record<string, FaqItem[]> = {
  ...
  en: [
    ...
    { q: 'Libemax has 200,000 downloads. Is GeoTapp reliable?', a: 'Libemax is an excellent attendance tracking app with a solid user base. GeoTapp solves a different problem: it seals work with tamper-evident evidence. Two different categories, like comparing a stopwatch with a notary.' },
    { q: 'Why are GeoTapp photos different from Libemax photos?', a: 'Libemax allows attaching photos to reports. GeoTapp seals every photo with a cryptographic hash chain at the moment of capture: if anyone modifies the photo by even one pixel, the seal breaks and the system detects it. GeoTapp photos are verifiable evidence, not simple attachments.' },
    { q: 'GeoTapp or Libemax for social cooperatives and cleaning companies?', a: 'For attendance tracking with NFC and geofence, Libemax is a solid choice. To eliminate client disputes with verifiable reports and tamper-evident evidence, GeoTapp is the only solution, because the client can check everything independently, without having to take your word for it.' },
  ],
  de: [
  ...
  fr: [
    ...
    { q: 'Libemax compte 200 000 téléchargements. GeoTapp est-il fiable ?', a: 'Libemax est une excellente application de pointage avec une base d\'utilisateurs solide. GeoTapp résout un problème différent : il scelle le travail avec des preuves dont toute modification est détectable. Deux catégories différentes, comme comparer un chronomètre à un notaire.' },
    { q: 'Pourquoi les photos de GeoTapp sont-elles différentes de celles de Libemax ?', a: 'Libemax permet de joindre des photos aux rapports. GeoTapp scelle chaque photo avec une chaîne de hachage cryptographique au moment même de la prise de vue : si quelqu\'un modifie la photo, ne serait-ce que d\'un pixel, le sceau se brise et le système le détecte. Les photos de GeoTapp sont des preuves vérifiables, pas de simples pièces jointes.' },
    { q: 'GeoTapp ou Libemax pour les coopératives sociales et les entreprises de nettoyage ?', a: 'Pour le simple pointage avec NFC et geofence, Libemax est un choix solide. Pour éliminer les litiges avec les clients grâce à des rapports vérifiables et des preuves dont toute modification est détectable, GeoTapp est la seule solution, car le donneur d\'ordre peut tout contrôler lui-même, sans devoir vous croire sur parole.' },
  ],
  es: [
  ...
  nl: [
    ...
    { q: 'Libemax heeft 200.000 downloads. Is GeoTapp betrouwbaar?', a: 'Libemax is een uitstekende app voor tijdregistratie met een stevige gebruikersbasis. GeoTapp lost een ander probleem op: het verzegelt werk met bewijs waarvan elke wijziging detecteerbaar is. Twee verschillende categorieën, alsof je een stopwatch met een notaris vergelijkt.' },
    { q: 'Waarom zijn de foto\'s van GeoTapp anders dan die van Libemax?', a: 'Libemax laat toe foto\'s aan rapporten toe te voegen. GeoTapp verzegelt elke foto met een cryptografische hashketen op het moment van de opname: wijzigt iemand de foto ook maar met één pixel, dan breekt het zegel en detecteert het systeem dit. De foto\'s van GeoTapp zijn verifieerbaar bewijs, geen simpele bijlagen.' },
    { q: 'GeoTapp of Libemax voor sociale coöperaties en schoonmaakbedrijven?', a: 'Voor enkel tijdregistratie met NFC en geofence is Libemax een solide keuze. Om klachten van klanten te elimineren met verifieerbare rapporten en bewijs waarvan elke wijziging detecteerbaar is, is GeoTapp de enige oplossing, want de opdrachtgever kan alles zelf controleren, zonder je op je woord te moeten geloven.' },
  ],
  da: [
...
const T: Record<string, Copy> = {
  ...
  en: {
    ...
    desc: 'Libemax is Italy\'s most downloaded attendance app with 200,000+ downloads. GeoTapp is a verifiable proof of work system with anti-spoofing GPS and tamper-evident reports. Two fundamentally different approaches to the same problem.',
    summary: 'Bottom line:',
    summaryText: 'Libemax excels at attendance tracking with multiple methods (GPS, QR, NFC, Bluetooth, geofence). But geofence only checks the perimeter, not if the position is real. GeoTapp goes further: anti-spoofing verifies GPS authenticity, photos are cryptographically sealed and reports are verifiable, the client verifies it independently.',
    ...
    geo: ['Anti-spoofing GPS: verifies the position is real','Photos sealed with cryptographic hash chain','Report with a verifiable cryptographic seal','The client verifies independently','Verifiable proof, not just attendance'],
    comp: ['200,000+ downloads, 6,000+ companies','GPS + QR + NFC + Bluetooth + geofence','Geofence: checks the perimeter (not GPS authenticity)','Checklists and audits for construction','Attendance tracking, not verifiable proof'],
  ...
  de: {
    ...
    desc: 'Libemax ist die meistgeladene Zeiterfassungs-App in Italien mit über 200.000 Downloads. GeoTapp ist ein System für versiegelte Arbeitsnachweise mit Anti-Spoofing-GPS und Berichten, bei denen jede Änderung erkennbar ist. Zwei grundlegend verschiedene Ansätze für dasselbe Problem.',
    summary: 'Fazit:',
    summaryText: 'Libemax glänzt bei der Zeiterfassung mit vielen Methoden (GPS, QR, NFC, Bluetooth, Geofence). Aber Geofence prüft nur den Perimeter, nicht ob die Position echt ist. GeoTapp geht weiter: Anti-Spoofing verifiziert das GPS, Fotos werden kryptographisch versiegelt und der Bericht ist überprüfbar, der Auftraggeber prüft es selbst.',
    ...
    geo: ['Anti-Spoofing-GPS: verifiziert echte Position','Fotos versiegelt mit kryptographischer Hash-Kette','Bericht mit überprüfbarem kryptographischem Siegel','Der Auftraggeber verifiziert selbst','Verifizierbarer Nachweis, nicht nur Zeiterfassung'],
    comp: ['200.000+ Downloads, 6.000+ Unternehmen','GPS + QR + NFC + Bluetooth + Geofence','Geofence: prüft Perimeter (nicht GPS-Echtheit)','Checklisten und Audits für Baustellen','Zeiterfassung, kein verifizierbarer Nachweis'],
  ...
  fr: {
    ...
    desc: 'Libemax est l\'application de pointage la plus téléchargée en Italie, avec plus de 200 000 téléchargements. GeoTapp est un système de preuve vérifiable du travail avec GPS anti-spoofing et rapports dont toute modification est détectable. Deux approches fondamentalement différentes du même problème.',
    summary: 'En résumé :',
    summaryText: 'Libemax excelle dans le pointage avec de multiples méthodes (GPS, QR, NFC, Bluetooth, geofence). Mais le geofence ne vérifie que le périmètre, pas si la position est réelle. GeoTapp va plus loin : l\'anti-spoofing vérifie que le GPS est authentique, les photos sont scellées cryptographiquement et le rapport est vérifiable, le client le vérifie lui-même.',
    ...
    geo: ['GPS anti-spoofing : vérifie que la position est réelle','Photos scellées par chaîne de hachage cryptographique','Rapport avec sceau cryptographique vérifiable','Le client vérifie lui-même','Preuve vérifiable, pas seulement pointage'],
    comp: ['200 000+ téléchargements, 6 000+ entreprises','GPS + QR + NFC + Bluetooth + geofence','Geofence : contrôle le périmètre (pas l\'authenticité GPS)','Checklists et audits pour les chantiers','Pointage, pas preuve vérifiable'],
  ...
  es: {
    ...
    desc: 'Libemax es la app de control de presencia más descargada de Italia, con más de 200.000 descargas. GeoTapp es un sistema de prueba verificable del trabajo con GPS anti-spoofing e informes cuya modificación es detectable. Dos enfoques radicalmente distintos para el mismo problema.',
    summary: 'En resumen:',
    summaryText: 'Libemax destaca en el control de presencia con múltiples métodos (GPS, QR, NFC, Bluetooth, geofence). Pero el geofence solo verifica el perímetro, no si la posición es real. GeoTapp va más allá: el anti-spoofing verifica que el GPS sea auténtico, las fotos se sellan criptográficamente y el informe es verificable, el cliente lo verifica por sí mismo.',
    ...
    geo: ['GPS anti-spoofing: verifica que la posición sea real','Fotos selladas con cadena hash criptográfica','Informe con sello criptográfico verificable','El cliente lo verifica por sí mismo','Prueba verificable, no solo control de presencia'],
    comp: ['200.000+ descargas, 6.000+ empresas','GPS + QR + NFC + Bluetooth + geofence','Geofence: controla el perímetro (no la autenticidad GPS)','Checklists y auditorías para obras','Control de presencia, no prueba verificable'],
  ...
  nl: {
    ...
    desc: 'Libemax is de meest gedownloade tijdregistratie-app van Italië, met meer dan 200.000 downloads. GeoTapp is een systeem voor verzegeld werkbewijs met anti-spoofing-GPS en rapporten waarvan elke wijziging detecteerbaar is. Twee fundamenteel verschillende benaderingen van hetzelfde probleem.',
    summary: 'Kort gezegd:',
    summaryText: 'Libemax blinkt uit in tijdregistratie met meerdere methoden (GPS, QR, NFC, Bluetooth, geofence). Maar geofence controleert alleen de perimeter, niet of de positie echt is. GeoTapp gaat verder: anti-spoofing verifieert dat de GPS authentiek is, foto\'s worden cryptografisch verzegeld en het rapport is verifieerbaar, de opdrachtgever verifieert het zelf.',
    ...
    ctaBtn: 'Begin gratis!',
    geo: ['Anti-spoofing-GPS: verifieert dat de positie echt is','Foto\'s verzegeld met cryptografische hashketen','Rapport met verifieerbaar cryptografisch zegel','De opdrachtgever verifieert zelf','Verifieerbaar bewijs, niet alleen tijdregistratie'],
    comp: ['200.000+ downloads, 6.000+ bedrijven','GPS + QR + NFC + Bluetooth + geofence','Geofence: controleert de perimeter (niet de GPS-echtheid)','Checklists en audits voor bouwplaatsen','Tijdregistratie, geen verifieerbaar bewijs'],
