...
import { getManutenzioneContent } from '@/content/settori/manutenzione';
import SettorePageLayout from '@/components/SettorePageLayout';
import BlogHighlights from '@/components/BlogHighlights';

const pathname = '/settori/manutenzione';
...
export async function generateMetadata({
  ...
  const content = await getManutenzioneContent(resolvedLocale);
...
export default async function ManutenzioneLocalePage({
  ...
  const breadcrumb = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 2, name: content.meta.title.replace(/ [|—].*$/, '').trim(), item: `https://geotapp.com/${resolvedLocale}/settori/manutenzione/` },
    ],
  };
  return (
    <>
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(breadcrumb) }} />
      <SettorePageLayout content={content} locale={resolvedLocale} settore="manutenzione" />
