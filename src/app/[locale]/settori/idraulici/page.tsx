...
export async function generateMetadata({
  ...
  const content = await getIdrauliciContent(resolvedLocale);
...
export default async function IdrauliciLocalePage({
  params,
}: {
  params: Promise<{ locale: string }>;
}) {
  ...
  const breadcrumb = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 2, name: content.meta.title.replace(/ [|—].*$/, '').trim(), item: `https://geotapp.com/${resolvedLocale}/settori/idraulici/` },
    ],
  };
  return (
    <>
