...
export default async function InstallatoriLocalePage({
  ...
  const breadcrumb = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 2, name: content.meta.title.replace(/ [|-].*$/, '').trim(), item: `https://geotapp.com/${resolvedLocale}/settori/installatori/` },
    ],
  };
  return (
    <>
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(breadcrumb) }} />
      <SettorePageLayout content={content} locale={resolvedLocale} settore="installatori">
