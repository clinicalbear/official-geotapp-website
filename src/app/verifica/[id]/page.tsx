      ...
      <div className="min-h-screen bg-slate-50 flex items-center justify-center px-6">
        <div className="max-w-md w-full text-center">
          ...
          <p className="text-slate-500 mb-6">
            The code <code className="bg-slate-100 px-2 py-1 rounded font-mono text-sm">{id}</code> does
            not match any report in the GeoTapp system.
          </p>
          <p className="text-sm text-slate-400 mb-8">
            Verify that the code is correct. If you believe this is an error, contact
    ...
    <div className="min-h-screen bg-slate-50 text-slate-900">
      ...
      <div className="bg-slate-900 text-white px-6 py-5">
        <div className="max-w-4xl mx-auto flex items-center justify-between">
          ...
          <span className="text-slate-400 text-sm hidden sm:block">Report Verifier</span>
        </div>
      </div>
      ...
      <div className="max-w-4xl mx-auto px-6 py-12 space-y-8">
        ...
        <div className={`rounded-2xl p-8 text-center border-2 ${verified ? 'bg-green-50 border-green-300' : 'bg-red-50 border-red-300'}`}>
          ...
          <div className={`text-2xl font-black mb-2 ${verified ? 'text-green-700' : 'text-red-700'}`}>
            {verified ? 'Authentic and verified report' : 'Verification failed'}
          </div>
          <p className={`text-sm ${verified ? 'text-green-600' : 'text-red-600'}`}>
            {verified
              ? 'This report was generated by GeoTapp, is cryptographically sealed and has not been modified after job closure.'
              : 'The integrity of this report could not be verified.'}
          </p>
        ...
        <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden shadow-sm">
          ...
          <div className="p-6 grid sm:grid-cols-2 gap-4">
            {[
              ['Job type', report.tipoIntervento],
              ['Category', report.categoria],
              ['Operator', report.operatorName],
              ['Client', report.clientName],
              ['Job ref.', report.commessaRef],
              ['Location', report.locationAddress],
              ['Location notes', report.locationNote],
        ...
        <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden shadow-sm">
          <div className="bg-slate-50 px-6 py-4 border-b border-slate-200 flex items-center gap-2">
            ...
            <span className="font-bold text-slate-800">Certified Clock-ins</span>
          ...
          <div className="p-6 grid sm:grid-cols-3 gap-4">
            <div className="bg-green-50 border border-green-200 rounded-xl p-5">
              <div className="text-xs font-bold text-green-600 uppercase tracking-wide mb-2">↓ Job start</div>
              <div className="text-2xl font-black text-green-700 mb-1">
                {new Date(report.inizioTs).toLocaleTimeString('en-GB', { hour: '2-digit', minute: '2-digit' })}
              </div>
            ...
            <div className="bg-slate-800 rounded-xl p-5 text-center flex flex-col items-center justify-center">
              <div className="text-xs text-slate-400 uppercase tracking-wide mb-2">Duration</div>
              <div className="text-3xl font-black text-white">{durataLabel(report.durataMinuti)}</div>
              <div className="text-xs text-slate-500 mt-1">hours worked</div>
            </div>
            <div className="bg-orange-50 border border-orange-200 rounded-xl p-5">
              <div className="text-xs font-bold text-orange-600 uppercase tracking-wide mb-2">↑ Job end</div>
              <div className="text-2xl font-black text-orange-700 mb-1">
                {new Date(report.fineTs).toLocaleTimeString('en-GB', { hour: '2-digit', minute: '2-digit' })}
              </div>
        ...
        <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden shadow-sm">
          <div className="bg-slate-50 px-6 py-4 border-b border-slate-200 flex items-center gap-2">
            ...
            <span className="font-bold text-slate-800">Photo Evidence</span>
            <span className="ml-auto text-xs bg-green-100 text-green-700 font-bold px-3 py-1 rounded-full">
              Hash verified
            </span>
          </div>
          <div className="p-6 grid sm:grid-cols-2 gap-6">
            {[
              { label: 'Initial state', tag: 'BEFORE', color: 'red', url: report.fotoInizioUrl, hash: report.fotoInizioHash, ts: report.fotoInizioTs },
              { label: 'Work completed', tag: 'AFTER', color: 'green', url: report.fotoFineUrl, hash: report.fotoFineHash, ts: report.fotoFineTs },
              ...
              <div key={tag} className={`rounded-xl overflow-hidden border-2 ${color === 'red' ? 'border-red-200' : 'border-green-200'}`}>
                ...
                <div className="p-3 bg-slate-50 border-t border-slate-100">
                  ...
                  <div className="flex items-center gap-1 mt-2">
                    ...
                    <span className="text-xs text-green-600 font-semibold">Hash intact — photo not modified</span>
                  </div>
                </div>
              </div>
            ))}
        ...
        <div className="bg-slate-900 rounded-2xl p-6 text-white">
          <div className="flex items-center gap-3 mb-4">
            ...
            <span className="font-bold text-lg">Cryptographic Seal</span>
            <span className="ml-auto text-xs bg-green-900 text-green-400 font-bold px-3 py-1 rounded-full">
              ✓ VALIDA
            </span>
          </div>
          <p className="text-slate-400 text-sm mb-4 leading-relaxed">
            The seal is calculated from all job data: operator ID,
            GPS coordinates, certified timestamps, and cryptographic hashes of every attached photo.
            Any modification after closure invalidates the seal.
          </p>
        ...
        <div className="text-center text-sm text-slate-400 pb-4">
          <p className="mb-2">
            ...
            {' '}— Field work certification system
          </p>
          <p className="text-xs">
            This document has evidentiary value. ID: {report.reportId}
          </p>
        </div>
