...
import {
    ArrowRight, Map, Camera,
    WifiOff, PenTool, Receipt,
    Clock, Shield, Navigation, Smartphone,
    X, Server, Globe, Cpu
} from 'lucide-react';
...
const SystemCard = ({ system, onClick }: { system: SystemDetail & { label_open?: string }, onClick: (s: SystemDetail) => void }) => {
    ...
    const getColorClasses = (color: string) => {
        switch (color) {
            ...
            case 'slate': return { text: 'text-slate-600', bg: 'bg-slate-100', letter: 'text-slate-600' };
        ...
        <div className="bg-background min-h-screen text-slate-900 pt-32 pb-24 overflow-x-hidden">
                    ...
                    >
                        ...
                        <div className="relative bg-slate-50 rounded-2xl shadow-2xl border border-slate-200 aspect-[16/9] flex items-center justify-center overflow-hidden">
                            <div className="text-center z-10">
                                <Smartphone size={80} className="text-blue-500 mx-auto mb-6 opacity-50" />
                                <div className="text-slate-300 font-mono text-3xl font-bold tracking-widest">{appDict.visual_placeholder}</div>
                            </div>
                        </div>
                    </motion.div>
                </motion.div>
            ...
            <section className="container mx-auto px-6 max-w-7xl">
                ...
                <div className="text-center max-w-3xl mx-auto mb-24">
                    <h2 className="text-3xl md:text-5xl font-display font-bold text-slate-900 mb-6">{appDict.grid_title}</h2>
                    <p className="text-xl text-slate-500">
                        {appDict.grid_subtitle}
                    </p>
                </div>
                ...
                <div className="grid md:grid-cols-3 gap-8 mb-8">
                    <SystemCard system={getSystem('timelock-alpha')} onClick={setSelectedSystem} />
                    <SystemCard system={getSystem('event-horizon')} onClick={setSelectedSystem} />
                    <SystemCard system={getSystem('unit-matrix')} onClick={setSelectedSystem} />
                </div>

                {/* SECTOR 2 */}
                <Sectiondivider title={appDict.sectors["2"]} />
                <div className="grid md:grid-cols-3 gap-8 mb-8">
                    <SystemCard system={getSystem('sector-grid')} onClick={setSelectedSystem} />
                    <SystemCard system={getSystem('energy-logistics')} onClick={setSelectedSystem} />
                    <SystemCard system={getSystem('neural-link')} onClick={setSelectedSystem} />
                </div>

                {/* SECTOR 3 */}
                <Sectiondivider title={appDict.sectors["3"]} />
                <div className="grid md:grid-cols-3 gap-8">
                    <SystemCard system={getSystem('payroll-bridge')} onClick={setSelectedSystem} />
                    <SystemCard system={getSystem('data-core')} onClick={setSelectedSystem} />
                    <SystemCard system={getSystem('identity-forge')} onClick={setSelectedSystem} />
                </div>
