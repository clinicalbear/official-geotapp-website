...
import { FORTYX_HEROES, HeroDetail } from './heros-data';
...
const RosterCard = ({ hero, onClick }: { hero: HeroDetail, onClick: (h: HeroDetail) => void }) => {
  ...
  return (
    <motion.div
      ...
      whileHover={{ y: -5 }}
      className="cursor-pointer p-6 bg-slate-50 rounded-xl border border-slate-100 hover:shadow-xl hover:border-fortyx/20 transition-all duration-300 group relative overflow-hidden"
    >
      ...
      <div className="mt-4 flex items-center text-fortyx font-bold text-xs uppercase tracking-wider opacity-0 group-hover:opacity-100 transition-opacity">
    ...
    <div className="bg-background min-h-screen text-text-primary pt-32 pb-24 overflow-x-hidden">
            ...
            >
              ...
              <div className="bg-slate-50 p-8 md:w-1/3 border-r border-slate-100 flex flex-col justify-between shrink-0">
                <div>
                  ...
                  <div className="w-16 h-2 bg-fortyx rounded-full mb-6"></div>
                </div>
                <div
                  className="text-xs text-slate-400 font-mono border-t border-slate-200 pt-4"
                  dangerouslySetInnerHTML={{ __html: fortyxDict.modal.db_header }}
                >
                </div>
              </div>
              ...
              <div className="p-8 md:p-12 md:w-2/3 prose prose-slate max-w-none relative">
                ...
                <div className="mt-12 pt-8 border-t border-slate-100">
                  <button
                    onClick={() => setSelectedHero(null)}
                    className="w-full py-4 bg-fortyx text-white font-bold rounded-xl hover:bg-fortyx/90 transition-colors shadow-lg shadow-fortyx/20"
                  >
                    {fortyxDict.modal.close}
                  </button>
                </div>
              </div>
            </motion.div>
      ...
      <section className="container mx-auto px-6 max-w-6xl pb-24 text-lg text-slate-700">
        ...
        <div className="text-center max-w-3xl mx-auto mb-24">
          <h2 className="text-3xl md:text-4xl font-display font-bold text-slate-900 mb-6">{fortyxDict.grid_title}</h2>
          <p className="leading-loose mb-6" dangerouslySetInnerHTML={{ __html: fortyxDict.grid_subtitle }}>
          </p>
        </div>

        {/* CLASS S: SPEED */}
        <Sectiondivider title={fortyxDict.classes.S} />
        ...
        {/* CLASS A: DEFENSE */}
        <Sectiondivider title={fortyxDict.classes.A} />
        <div className="grid md:grid-cols-3 gap-6 mb-16">
        ...
        <Sectiondivider title={fortyxDict.classes.B} />
        <div className="grid md:grid-cols-3 gap-6 mb-16">
        ...
        {/* CLASS C: REALITY */}
        <Sectiondivider title={fortyxDict.classes.C} />
        ...
        <div className="text-center py-24">
          <h2 className="text-3xl font-bold mb-8">{fortyxDict.cta_title}</h2>
          <Link href={getLink("/pricing")} className="inline-flex items-center gap-3 px-8 py-4 bg-fortyx text-white font-bold rounded-xl text-xl hover:bg-fortyx/90 transition-all shadow-lg hover:shadow-fortyx/30 transform hover:-translate-y-1">
            {fortyxDict.cta_button} <ArrowRight />
          </Link>
        </div>
